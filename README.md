...
当前源模型已进入编带防脱 v6：标签纸仍为 `25 × 9 mm`，上、下压边覆盖量约 `2.8 mm`，压边材料厚度 `1.0 mm`，标签凹槽深度 `0.35 mm`，不依赖胶粘。标签槽已镜像为右侧开口，标签从右向左插入，左端直接由活页孔内侧的整块加强高台止挡；标签槽向左移动后，释放出的空间全部让给 SMD 滑道。编带内宽约 `8.6 mm`，双侧 T 形压边每侧覆盖约 `1.3 mm`，顶部有效开口约 `6.0 mm`；相邻滑道用约 `0.8 mm` 共用薄墙直接相接，页面增加到 18 条滑道。新版 STL 使用 `_v6_08mm_partition` 文件名，先打印测试片确认实际插入和防脱手感。

当前工作树在 v6 基础上保留连续外圈围墙：围墙高约 `3.45 mm`、平面内墙厚 `2.0 mm`，从页面最外缘向内形成加强边，既帮助装订后保持页面平整，也作为每条编带滑道的右端止挡。左侧装订区域从页面边缘到孔列内侧加强脊全部满铺到同一高度，孔洞贯穿高台，以提升装订孔抗撕裂能力。新版导出文件使用 `_v6_08mm_partition` 文件名。
...
"$OPENSCAD" --export-format binstl \
  -o output/3d-print/0603_tape_page_v6_08mm_partition.stl \
  src/0603_tape_page.scad

"$OPENSCAD" --export-format binstl \
  -o output/3d-print/0603_fit_coupon_dual_t_v6_08mm_partition.stl \
  src/0603_fit_coupon_dual_t.scad

"$OPENSCAD" --export-format binstl \
  -o output/3d-print/0603_fit_coupon_single_7_v6_08mm_partition.stl \
  src/0603_fit_coupon_single_7.scad
```

右侧标签入口 + 装订边满铺加强 + 0.8 mm 薄隔档版 v6 输出：

```text
output/3d-print/0603_tape_page_v6_08mm_partition.stl
output/3d-print/0603_tape_page_v6_08mm_partition_x1c_safe.stl
output/3d-print/0603_fit_coupon_dual_t_v6_08mm_partition.stl
output/3d-print/0603_fit_coupon_single_7_v6_08mm_partition.stl
```
...
该 `.3mf` 是上一版 B5 工程，不能代表当前 A5 v6 几何；正式打印前应在 Bambu Studio 中导入新的 v6 STL，确认 X1C 当前装的是干燥 PETG、纹理 PEI 板已清洁，并先用双 T 测试片确认实际编带和右侧标签配合。
